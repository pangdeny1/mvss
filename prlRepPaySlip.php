...
/* Standard PDF file creation header stuff */
	...
	$sql = "SELECT employeeid,basicpay,othincome,absent,areaspay,late,otpay,grosspay,loandeduction,sss,hdmf,philhealth,tax,netpay
	...
	if(DB_num_rows($PayResult)>0)
	{
		while ($myrow=DB_fetch_array($PayResult)) {
			...
			if ($PaySlip==1) {
				...
				$YPos -= $line_height;
				$LeftOvers = $pdf->addTextWrap(155,$YPos,65,$FontSize,': ','right');
				$LeftOvers = $pdf->addTextWrap(221,$YPos,40,$FontSize,number_format($HDMF,2),'right');
				$YPos -= $line_height;
				$LeftOvers = $pdf->addTextWrap(155,$YPos,65,$FontSize,'NHIF : ','right');
				...
				$YPos -= $line_height;
				$LeftOvers = $pdf->addTextWrap(432,$YPos,65,$FontSize,'DOSHITWU : ','right');
				$LeftOvers = $pdf->addTextWrap(498,$YPos,40,$FontSize,number_format($HDMF,2),'right');
				$YPos -= $line_height;
				$LeftOvers = $pdf->addTextWrap(432,$YPos,65,$FontSize,'NHIF : ','right');
	...
	echo '<CENTER><TABLE><TR><TD>' . _('Select Employee:') . '</TD><TD><SELECT Name="EmployeeID">';
	...
	echo '</SELECT></TD></TR>';
	echo "</TABLE><P><CENTER><INPUT TYPE='Submit' NAME='ShowPR' VALUE='" . _('Show Pay Slip') . "'>";
